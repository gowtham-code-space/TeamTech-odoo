  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Resource Bookings</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Reserve shared resources, testing equipment, and meeting rooms</p>
        </div>
      ...
      {showAddForm && (
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm relative animate-fade-in space-y-4">
          <h3 className="text-base font-bold text-slate-800 dark:text-white">Create Booking Request</h3>
          <form onSubmit={handleCreateBooking} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Resource (resource_id)</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Start Time (start_time)</label>
              <input
                type="datetime-local"
                required
                value={startTime}
                onChange={(e) => setStartTime(e.target.value)}
                className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">End Time (end_time)</label>
              <input
                type="datetime-local"
                required
                value={endTime}
            ...
            <div className="md:col-span-2 lg:col-span-3">
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2">Purpose of Booking</label>
              <textarea
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 dark:bg-rose-500/5 border border-rose-200 dark:border-rose-500/20 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800 dark:text-rose-400">Failed to Resolve Bookings</h3>
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-center">
          <RiCalendarEventLine className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
            ...
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {bookings.map((booking) => (
                  <tr key={booking.booking_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-500 dark:text-slate-400">{booking.booking_id}</td>
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-800 dark:text-slate-100">{booking.resource_name}</p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs mt-0.5">{booking.resource_id}</p>
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300 font-semibold">{formatDate(booking.start_time)}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300 font-semibold">{formatDate(booking.end_time)}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-slate-400 dark:text-slate-500 text-xs font-semibold">{formatDate(booking.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
