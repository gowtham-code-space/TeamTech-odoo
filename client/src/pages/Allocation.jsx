  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Asset Allocations</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Track and audit active allocations, lease durations, and returns</p>
        </div>
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 dark:bg-rose-500/5 border border-rose-200 dark:border-rose-500/20 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800 dark:text-rose-400">Failed to Retrieve Allocations</h3>
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-center">
          <RiExchangeLine className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">No Allocations Found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {allocations.map((alc) => (
                  <tr key={alc.allocation_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-500 dark:text-slate-400">{alc.allocation_id}</td>
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-800 dark:text-slate-100">{alc.asset_name}</p>
                      <p className="text-slate-400 dark:text-slate-500 text-xs mt-0.5">{alc.asset_id}</p>
                    </td>
                    <td className="px-6 py-4 font-semibold text-slate-700 dark:text-slate-200">{alc.user_name}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{alc.allocated_by}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400 font-semibold">{formatDate(alc.start_date)}</td>
                    <td className="px-6 py-4">
                      {alc.return_date ? (
                        <span className="text-slate-500 dark:text-slate-400 font-semibold">{formatDate(alc.return_date)}</span>
                      ) : (
                        <span className="text-emerald-600 dark:text-emerald-400 font-bold italic">Indefinite</span>
                      )}
                    </td>
                  </tr>
                ))}
