  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Organization Structure</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage departments, view headcount, and monitor asset allocation budgets</p>
        </div>
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 dark:bg-rose-500/5 border border-rose-200 dark:border-rose-500/20 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800 dark:text-rose-400">Failed to Retrieve Organization Structure</h3>
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-center">
          <RiBuilding4Line className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">No Departments Found</h3>
            ...
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {departments.map((dept) => (
                  <tr key={dept.dept_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-500 dark:text-slate-400">{dept.dept_id}</td>
                    <td className="px-6 py-4 font-bold text-slate-800 dark:text-slate-100">{dept.dept_name}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{dept.head}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300 font-semibold">{dept.count} members</td>
                    <td className="px-6 py-4 font-semibold text-indigo-600 dark:text-indigo-400">{dept.budget}</td>
                  </tr>
                ))}
              </tbody>
            </table>
