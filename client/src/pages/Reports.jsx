    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Generate, view, and export asset metrics, depreciation, and financial audit files</p>
        </div>
      </div>
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 dark:bg-rose-500/5 border border-rose-200 dark:border-rose-500/20 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800 dark:text-rose-400">Failed to Retrieve System Reports</h3>
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-center">
          <RiPieChartLine className="w-16 h-16 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">No Reports Found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {reports.map((rpt) => (
                  <tr key={rpt.report_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-500 dark:text-slate-400">{rpt.report_id}</td>
                    <td className="px-6 py-4 font-bold text-slate-800 dark:text-slate-100">{rpt.title}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{rpt.author}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300 font-semibold">{rpt.size}</td>
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400 font-bold">{rpt.type}</td>
                    <td className="px-6 py-4">
                      <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-lg text-xs transition-all shadow-sm cursor-pointer">
                        <RiFileDownloadLine className="w-4 h-4 text-slate-400 dark:text-slate-500" />
                        <span>Download</span>
                      </button>
                    </td>
                  </tr>
