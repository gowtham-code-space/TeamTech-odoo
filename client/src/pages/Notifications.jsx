    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Stay updated on your booking requests, asset allocations, and compliance alerts</p>
        </div>
        
        {viewState === 'success' && notifications.some(n => !n.is_read) && (
          <button
            onClick={handleMarkAllRead}
            className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-250 text-slate-700 hover:text-slate-900 border border-slate-200 rounded-xl font-bold text-xs shadow-sm cursor-pointer transition-all"
          >
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 border border-rose-200 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800">Failed to Retrieve Notifications</h3>
      ...
      {viewState === 'loading' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-4 animate-pulse">
          {[1, 2].map((i) => (
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 bg-white text-center">
          <RiNotificationLine className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          ...
          {notifications.map((ntf) => (
            ...
            >
              ...
              <div className={`p-2.5 rounded-xl shrink-0 ${
                ntf.is_read ? 'bg-slate-100 text-slate-500' : 'bg-indigo-50 text-indigo-600 border border-indigo-100'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-block px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-slate-100 border border-slate-200 text-slate-600 rounded">
                    {ntf.notification_type.replace('_', ' ')}
                  </span>
                  <span className="text-[10px] font-semibold text-slate-400">{formatDate(ntf.created_at)}</span>
                </div>
                ...
                {!ntf.is_read && (
                  <button
                    ...
                    className="mt-3 text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors cursor-pointer"
                  >
                    Mark as read
                  </button>
                )}
