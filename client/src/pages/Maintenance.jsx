  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-950 tracking-tight">Maintenance Logs</h2>
          <p className="text-sm text-slate-500 mt-0.5">Report defects, track repairs, and manage scheduled testing</p>
        </div>
      ...
      {showAddForm && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative animate-fade-in space-y-4">
          <h3 className="text-base font-bold text-slate-800">File Maintenance Request</h3>
          <form onSubmit={handleCreateRequest} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Asset ID (asset_id)</label>
              <input
                type="text"
                required
                placeholder="ast_101"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Priority (priority)</label>
              <select
                value={priority}
            ...
            <div className="md:col-span-2 lg:col-span-3">
              <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Issue Description</label>
              <textarea
                required
            ...
            <div className="md:col-span-2 lg:col-span-3 flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-slate-500 hover:text-slate-700 font-bold text-sm cursor-pointer"
              >
                Cancel
              </button>
              <button
      ...
      {viewState === 'error' && (
        <div className="p-12 rounded-2xl bg-rose-50 border border-rose-200 text-center space-y-4">
          ...
          <h3 className="text-lg font-bold text-rose-800">Failed to Resolve Maintenance Requests</h3>
      ...
      {viewState === 'empty' && (
        <div className="p-12 rounded-2xl border-2 border-dashed border-slate-200 bg-white text-center">
          <RiToolsLine className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          <h3 className="text-lg font-bold text-slate-800">No Maintenance Found</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {requests.map((req) => (
                  <tr key={req.request_id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-600">{req.request_id}</td>
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-800">{req.asset_name}</p>
                      <p className="text-slate-400 text-xs mt-0.5">{req.asset_id}</p>
                    </td>
                    <td className="px-6 py-4 text-slate-600 font-medium max-w-xs truncate" title={req.issue_description}>
                    ...
                    <td className="px-6 py-4 text-slate-400 text-xs font-semibold">{formatDate(req.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
