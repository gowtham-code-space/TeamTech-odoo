  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Maintenance Logs</h2>
      ...
      {showAddForm && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative animate-fade-in space-y-4">
          ...
          <form onSubmit={handleCreateRequest} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Asset ID (asset_id)</label>
              <input
                type="text"
                required
                placeholder="ast_101"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Priority (priority)</label>
              <select
                value={priority}
              ...
              <tbody className="divide-y divide-slate-100">
                {requests.map((req) => (
                  <tr key={req.request_id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-semibold text-slate-500">{req.request_id}</td>
