...
import { UserPlus, UserMinus, Wrench, RefreshCw, ClipboardCheck, ArrowLeftRight, Clock, AlertTriangle } from 'lucide-react';
...
/**
 * AssetHistory Timeline Component
 *
 * @component
 * @param {Object} props
 * @param {Object} props.asset - Target asset metadata
 * @param {Array<Object>} props.history - List of timeline events from backend
 * @param {boolean} props.loading - Timeline fetch state
 * @param {string} [props.error] - Timeline fetch error message
...
export default function AssetHistory({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6 text-left max-w-4xl mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-105 pb-4 gap-4">
        <div>
      ...
      {loading ? (
        ...
        <div className="py-12 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center text-slate-400">
          ...
          <p className="text-sm font-semibold text-slate-500">No History Records Registered</p>
          <p className="text-xs text-slate-400 mt-0.5">Activities appear as allocations, checkups, or edits occur.</p>
        </div>
      ) : (
            ...
            return (
              <div key={event.id || index} className="relative pl-8 group">
                ...
                <div className="space-y-1.5 bg-slate-50/50 hover:bg-slate-50 p-4 rounded-xl border border-slate-150 transition-colors">
                  ...
                  {event.performed_by && (
                    <div className="flex items-center gap-1.5 pt-1.5 border-t border-slate-200/50 text-[10px] text-slate-400 font-semibold uppercase tracking-wider">
                      <span>Operator:</span>
                      <span className="text-slate-600 font-bold">{event.performed_by?.name || event.performed_by}</span>
                    </div>
                  )}
                </div>
              </div>
